...
type lpkv = 
    static member serialize (items: struct (ReadOnlyMemory<byte> * ReadOnlyMemory<byte>) seq) : byte[] =
    ...
    [<MethodImpl(MethodImplOptions.AggressiveInlining)>]
    static member lookup (key: ReadOnlySpan<byte>, data: ReadOnlySpan<byte>) : ReadOnlySpan<byte> =
    ...
    [<MethodImpl(MethodImplOptions.AggressiveInlining)>]
    static member lookup (key: ReadOnlySpan<byte>, data: ReadOnlyMemory<byte>) : ReadOnlyMemory<byte> =
        let mutable result = ReadOnlyMemory<byte>.Empty
        let mutable pos = 0
        let span = data.Span
        while pos < data.Length do
            let keyLen = MemoryMarshal.Read<uint32>(span.Slice(pos, 4))
            let valueLen = MemoryMarshal.Read<uint32>(span.Slice(pos + 4, 4))
            if (pos + int (keyLen + valueLen)) > data.Length then
                failwith "eof reading value len"
            if keyLen = uint32 key.Length && span.Slice(pos + 8, int keyLen).SequenceEqual(key) then
                result <- data.Slice(pos + 8 + int keyLen, int valueLen)
                pos <- data.Length
            else
                pos <- pos + 8 + int keyLen + int valueLen
